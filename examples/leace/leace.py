...
class LeaceEraser:
    ...
    def __init__(self, act_data):
        """Erasure function lazily computed given the current statistics."""
        eye = torch.eye(act_data.x_dim, device=act_data.mean_x.device, dtype=act_data.mean_x.dtype)

        # Compute the whitening and unwhitening matrices
        if act_data.method == "leace":
            sigma = act_data.sigma_xx
        ...
        u, s, _ = torch.linalg.svd(W @ act_data.sigma_xz, full_matrices=False)

        # Throw away singular values that are too small
        u *= s > act_data.svd_tol

        proj_left = W_inv @ u
        proj_right = u.mH @ W

        if act_data.constrain_cov_trace and act_data.method == "leace":
